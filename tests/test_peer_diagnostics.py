...
def test_two_peer_runner_approval_media_recording_input_and_revocation(tmp_path):
    async def scenario():
        ...
        try:
            ...
            receiver = asyncio.create_task(client({'address': f"127.0.0.1:{ready['port']}",
                                                   'invitation': ready['invitation'], 'name': 'peer-test',
                                                   'width': 640, 'height': 360, 'fps': 30, 'seconds': 2,
                                                   'wait_revoke': True}, client_root, lambda _: None))
            await until(lambda: (client_root / 'awaiting-revoke.json').exists(), 20)
            (host_root / 'revoke').touch()
            ...
            assert host_report['ok'] and host_report['cleanup'] and host_report['revoked']
            assert client_report['ok'] and client_report['cleanup'] and client_report['revocation_disconnected']
