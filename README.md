...
串流画面左上角显示六行紧凑统计：实际路径、FPS、接收 Mbps、网络往返 RTT、帧间隔和视频丢包率。F10 可隐藏。未知数据显示 `--`；Tailscale 直连或中继通过实际探测区分。Mbps 是实时接收速率，不是设定的码率上限；帧间隔不代表端到端延迟。详细测量口径见 [串流统计说明](docs/stream-statistics.md)。
